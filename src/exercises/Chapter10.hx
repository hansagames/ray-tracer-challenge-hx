...
class Chapter10 {
	public function new() {
		...
		File.saveContent(Path.join(["build", "chapter10.ppm"]), camera.render(world).toPPM());
	}
	...
	function createMiddleSphere() {
		...
		final pattern = gradientPattern(createColor(0, 0, 0), createColor(1, 1, 1));
	...
	function createLeftSphere() {
		...
		sphere.material.diffuse = 0.7;
		sphere.material.specular = 0.3;
		final pattern = stripePattern(createColor(1, 0, 0), createColor(0, 1, 0));
		pattern.transform = rotationY(Math.PI * 0.25);
		sphere.material.pattern = pattern;
		return sphere;
	}
